...
- **集中配置：** 通过 SQLite 应用状态统一管理端点配置，API 密钥可存储在 provider 记录中或通过 `.env`/环境变量提供。
...
使用 SQLite provider store 初始化并添加 provider：

```json
cam provider init
cam provider add my-litellm --endpoint https://api.example.com:4142 --api-key-env API_KEY_LITELLM --client claude,codex,qwen,copilot --description "我的 LiteLLM 代理"
```
