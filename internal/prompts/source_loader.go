...
func (s *Service) loadPromptCSV(ctx context.Context, repo catalogconfig.GitHubRepo, source catalogconfig.CatalogSource) ([]AwesomePrompt, error) {
	...
	for idx, row := range rows[1:] {
		...
		out = append(out, AwesomePrompt{
			...
			Source:      catalogconfig.SourceSlug(source.Name),
...
func promptFromFile(source catalogconfig.CatalogSource, repo catalogconfig.GitHubRepo, filePath string, data []byte) (AwesomePrompt, bool) {
	...
	case strings.HasSuffix(lower, ".txt"):
		...
		return AwesomePrompt{Slug: slugFromTitle(title), Title: title, Prompt: content, Category: path.Base(path.Dir(filePath)), Source: catalogconfig.SourceSlug(source.Name), SourceURL: repo.BlobURL(filePath)}, true
	}
	return AwesomePrompt{}, false
}
...
func promptFromStructured(source catalogconfig.CatalogSource, repo catalogconfig.GitHubRepo, filePath string, data []byte, isYAML bool) (AwesomePrompt, bool) {
	...
	return AwesomePrompt{Slug: slug, Title: title, Description: stringField(raw, "description"), Prompt: content, Tags: tagsField(raw, "tags"), Category: stringField(raw, "category"), Author: stringField(raw, "author"), Source: catalogconfig.SourceSlug(source.Name), SourceURL: repo.BlobURL(filePath)}, true
}
...
func promptFromMarkdown(source catalogconfig.CatalogSource, repo catalogconfig.GitHubRepo, filePath string, data []byte) (AwesomePrompt, bool) {
	...
	return AwesomePrompt{Slug: slugFromTitle(title), Title: title, Description: desc, Prompt: content, Category: path.Base(path.Dir(filePath)), Source: catalogconfig.SourceSlug(source.Name), SourceURL: repo.BlobURL(filePath)}, true
}
